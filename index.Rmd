...
#Darkstarcompare %>% mutate(timbre = map(segments, compmus_summarise, timbre, method = 'mean')) %>% select(genre, timbre) %>% compmus_gather_timbre %>% ggplot(aes(x = basis, y = value, fill = genre)) + geom_violin() +scale_fill_viridis_d() + labs(x = 'Spotify Timbre Coefficients', y = '', fill = 'Period') + ggtitle("Plot of Variation in Tempo between Dark Star Recordings")    
...
In the charting of acousticness and instrumentalness, the two tangible variables, we do indeed see that that 1977, in general, was a less acoustic year during the 1975-1979 period, for both songs - on top of that, despite being a vocal song, a higher rate of St. Stephen recordings are classified as not non-instrumental during that period.
